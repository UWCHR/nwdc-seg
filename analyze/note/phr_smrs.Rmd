...
The dataset includes a total of `r nrow(df %>% filter(facility == nwdc))` SRMS placement records for the Northwest Detention Center (NWDC) in Tacoma, WA.
...
t1 <- df %>% 
  ...
  dplyr::select(country_of_citizenship, facility, length_of_stay, ongoing_stay) %>% 
...
SRMS placements are flagged for "special vulnerability" categories, noted in the dataset as "Mental Illness", "Serious Medical Illness", "Serious Disability", and "Suicide Risk?".

```{r mental_illness_prop}

# Standardize one bad value
df <- df %>% 
